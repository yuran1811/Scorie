...
export const FeatureCard: FC<FeatureCardProps & DivProps> = ({ title = '', className = '', desc = '', ...otherProps }) => {
  ...
  return (
    <div {...otherProps} className={`scrollY w-full cursor-pointer rounded-[1.6rem] text-white ${className}`}>
      {!!title.length && (
        <div className="typo-sm sticky left-0 top-0 line-clamp-2 bg-inherit px-4 pt-6 text-center font-bold">
          {t(title.toLowerCase())}
        </div>
      )}

      {!!desc.length && (
        <div className="scrollY typo-3sm text-center font-medium">
